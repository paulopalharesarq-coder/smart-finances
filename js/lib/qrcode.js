/**
 * Micro QR Code Generator (Zero-dependency SVG/Canvas QR Renderer)
 * Adapted for offline PWA & local network phone pairing.
 */
(function (global) {
  // Simple, self-contained QR code generation library (Type 1 to 10, ECC Level L/M)
  ...
  QRCodeModel.prototype = {
    ...
    make: function () {
      if (this.typeNumber < 1) {
        ...
        for (typeNumber = 1; typeNumber < 10; typeNumber++) {
          ...
          if (totalBytes <= totalDataCount - 4) break;
        }
        this.typeNumber = typeNumber;
      }
    ...
    getBestMaskPattern: function () {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        const lostPoint = QRUtil.getLostPoint ? QRUtil.getLostPoint(this) : 0;
  ...
  // Export helper function to generate SVG string
  global.generateQRCodeSVG = function (text, size = 200, color = "#231a13", bgColor = "#ffffff") {
    try {
      ...
      svg += `<rect width="${size}" height="${size}" fill="${bgColor}" rx="12"/>`;
